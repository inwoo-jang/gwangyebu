...
interface Props {
  ...
  personName: string
  /**
   * 사용자 본인의 카톡 표시명. inbound/outbound 판정에 사용.
   * 없어도 동작은 함 (모두 inbound 처리).
   */
  selfName?: string
}

type ImportResult = { kind: "txt"; days: number; logs: number } | { kind: "jpg"; ok: boolean }

/**
 * 카카오톡 내보내기 .txt 또는 카톡 스크린샷 .jpg 업로드 → 연락 기록 자동 등록.
 * - .txt: 날짜 자동 파싱 + AI 요약 (실패 시 룰베이스 폴백)
 * - .jpg: 사용자가 날짜 입력 + AI Vision 요약 (실패 시 사용자가 직접 메모)
 */
export function GuestImportContacts({ personId, personName, selfName }: Props) {
  ...
  const handleTxt = async (file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < days.length; i++) {
        ...
        const direction = day.outboundCount >= day.inboundCount ? "outbound" : "inbound"
        // 시간은 그날 마지막 메시지 시각으로
        const occurredAt =
